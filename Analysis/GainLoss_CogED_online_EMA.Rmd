---
title: "Gain-Loss CogED + EMA Online"
author: "JC"
date: "10/03/2021"
output:
...
coged.path<-"~/Box/CCPLab_Aging_Studies/OA_Effort_EMA/Data/Online/CogED/coged_ema_gainloss.csv"
PTQ.path <- "~/Box/CCPLab_Aging_Studies/OA_Effort_EMA/Data/Online/CogED/coged_ema_ptq.csv"
EMA.path <- "~/Box/CCPLab_Aging_Studies/OA_Effort_EMA/Data/Online/EMA/Online_CogED_EMA.csv"
...
NASA_effort_sum$Task <- factor(NASA_effort_sum$Task, levels = c(1,2,3,4), labels = c("1-Back","2-Back","3-Back","4-Back"))

p.effort <- ggplot(NASA_effort_sum, aes(x=Task, y=rating, fill=agegroup)) + 
  theme(plot.title = element_text(hjust = 0.5), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line = element_line(colour = "black")) +
  geom_bar(stat="identity", position=position_dodge()) +
  geom_errorbar(position=position_dodge(width=0.9), aes(ymin=rating-ci, ymax=rating+ci), width=.2) +  
  xlab("Task") + ylab("Frustration") + ggtitle("Self-Reported Effort") 
...
codebook.path <- "~/Box/CCPLab_Aging_Studies/OA_Effort_EMA/Data/Online/EMA/Codebook.csv"
...
## Completion rate (42 total surveys)

``` {r completion, echo = TRUE}
completion<- EMA.clean %>% count(SubID)
kable(completion)
```
...
current.affect <- EMA.clean %>% select(SubID,agegroup, starts_with("affect_")) %>%
  pivot_longer(names_to = "Affect", values_to = "rating", -c(SubID, agegroup)) %>%
  separate(col = Affect, into=c(NA,"Affect_Term"), sep = "_") 

#summarizing average affect across all participants
affect.subj <-current.affect %>% group_by(SubID,agegroup, Affect_Term) %>% 
  dplyr::summarise(mean.affect = mean(rating, na.rm = T))

affect.av <-current.affect %>% group_by(Affect_Term) %>%
  dplyr::summarise(mean.affect = mean(rating, na.rm = T))
affect.sum <- summarySEwithin2(affect.subj, measurevar = "mean.affect", withinvars = c("Affect_Term"), betweenvars = "agegroup", idvar = "SubID")
...
p.affect <- ggplot(affect.sum, aes(x = Affect_Term, y= mean.affect, fill=agegroup)) +
 theme(plot.title = element_text(hjust = 0.5), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line = element_line(colour = "black")) +
  geom_bar(stat="identity", position=position_dodge()) +
...
p.affect + labs(fill = "Age Group") + scale_fill_brewer(palette = "Paired")
```
...
m.demand <- EMA.clean %>% select(SubID,agegroup, starts_with("demand_")) %>%
  pivot_longer(names_to = "category", values_to = "rating", -c(SubID, agegroup)) %>%
  separate(col = category, into=c(NA,"Category"), sep = "_") 

#summarizing average mental demand of activities across all participants
m.demand.subj <-m.demand %>% group_by(SubID,agegroup,Category) %>% filter(Category != "school") %>%
  dplyr::summarise(mean.demand = mean(rating, na.rm = T))

m.demand.subj.av <-m.demand %>% group_by(SubID, agegroup) %>% filter(Category != "school") %>%
  dplyr::summarise(mean.demand = mean(rating, na.rm = T))

m.demand.av <-m.demand %>% group_by(Category) %>%
  dplyr::summarise(mean.demand = mean(rating, na.rm = T))
m.demand.sum <- summarySEwithin2(m.demand.subj, measurevar = "mean.demand", withinvars = c("Category"), betweenvars = "agegroup", idvar = "SubID")
...
p.mental.demand.subj <- ggplot(m.demand.subj.av, aes(x = SubID, y= mean.demand, fill=agegroup)) +
 theme(plot.title = element_text(hjust = 0.5), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line = element_line(colour = "black")) +
  geom_bar(stat="identity", position=position_dodge()) +
    coord_cartesian(ylim=c(1,3)) +
  xlab("ID") + ylab("Mental Demand") + ggtitle("Average Mental Demand of Each Participant")
p.mental.demand.subj + labs(fill = "Age Group") + scale_fill_brewer(palette = "Paired")
...
Cog_ED_ind_SV$SubID = Cog_ED_ind_SV$subjectid
CogED.EMA <- inner_join(Cog_ED_ind_SV, m.demand.subj.av, by="SubID")

#Plotting SV Gain and average mental demand across both YAs and OAs
p.SV.EMA <-  ggplot(CogED.EMA, aes(meanSV_gain, mean.demand)) +
   theme(plot.title = element_text(hjust = 0.5), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line = element_line(colour = "black")) +
  geom_point(aes(colour = agegroup.x)) + geom_smooth(method=lm) +ggtitle("Cog-ED and Daily Life Effort") +
    xlab("SV Gain") + ylab("Average Mental Demand")
p.SV.EMA + scale_colour_brewer(palette = "Paired", "Age Group")

#correlation between CogED & EMA demand
cor.test(CogED.EMA$meanSV_gain, CogED.EMA$mean.demand)
```