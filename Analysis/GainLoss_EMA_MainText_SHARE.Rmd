...
## Exploratory Analyses: Mediation analysis

```{r mediation, warning=FALSE, message=FALSE}
coged.mediation <- Cog_ED_ind_SV %>% inner_join(m.demand.subj.av) %>%
   mutate(ageCode = factor(ageGroup, levels = c("Young", "Old"), labels = c(0,1)))
coged.mediation$ageCode <- as.numeric(as.character(coged.mediation$ageCode))

results.mediation.parallel = mediate(mean.demand ~ ageCode + (meanSV_gain) + (meanSV_loss), data = coged.mediation, n.iter = 5000)
results.mediation.parallel
summary(results.mediation.parallel)
```